...
# start editing here. these logicals are not right

```{r}
null_para_direct <- null_para_df %>% filter(close == TRUE | nonclose == TRUE)
null_para_indirect <- null_para_df %>% filter(vector == TRUE | intermediate == TRUE)
count(null_para_direct) + count(null_para_indirect) == count(null_para_df) #test
#check this but it is probably false because transmission mode data is not available for every parasite

sum(null_para_direct$vector) #should be 0
sum(null_para_direct$intermediate) #should be 0
sum(null_para_indirect$close) #should be 0
sum(null_para_indirect$nonclose) #should be 0
```
