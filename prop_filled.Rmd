...
max_rich <- null_para_df %>% make_mat() %>% get_richness_df() 

max_rich %<>% left_join(., actual_para_rich, by=c("ecoreg", "host"))

prop_rich <- max_rich %>% mutate(para.y = replace_na(para.y, 0),
                                 ratio = para.y/para.x)

saveRDS(prop_rich, file = here("data", "processed_data", "ratios", "prop_max_rich_ecoreg.rds"))
```
...
para_binary_rich %<>% mutate(para.y = replace_na(para.y, 0),
                             ratio = para.y/para.x)
...
saveRDS(type_ratios, file = here("data", "processed_data", "ratios", "prop_type_rich_ecoreg.rds"))
saveRDS(mode_ratios, file = here("data", "processed_data", "ratios", "prop_mode_rich_ecoreg.rds"))
saveRDS(spec_ratios, file = here("data", "processed_data", "ratios", "prop_spec_rich_ecoreg.rds"))
saveRDS(prev_ratios, file = here("data", "processed_data", "ratios", "prop_prev_rich_ecoreg.rds"))
saveRDS(int_ratios, file = here("data", "processed_data", "ratios", "prop_int_rich_ecoreg.rds"))
```
