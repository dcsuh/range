...
Why are there positive ratios when we are dividing observed in GMPD over max possible richness per ecoregion?
 ...
 - Try a paired test since data are paired by ecoregion?
 
 Do this again but for the proportion of ecoregions a parasite actually fills over the number it could fill. Currently this is for actual richness/theoretical richness
...
```{r}
paired_type %>% filter(ratio.x < 1 & ratio.y < 1) %>% ggplot(., aes(x=ratio.x, y=ratio.y)) + geom_point() + geom_abline(intercept=0)
paired_mode %>% filter(ratio.x < 1 & ratio.y < 1) %>% ggplot(., aes(x=ratio.x, y=ratio.y)) + geom_point() + geom_abline(intercept=0)
paired_spec %>% filter(ratio.x < 1 & ratio.y < 1) %>% ggplot(., aes(x=ratio.x, y=ratio.y)) + geom_point() + geom_abline(intercept=0)
paired_prev %>% filter(ratio.x < 1 & ratio.y < 1) %>% ggplot(., aes(x=ratio.x, y=ratio.y)) + geom_point() + geom_abline(intercept=0)
paired_int %>% filter(ratio.x < 1 & ratio.y < 1) %>% ggplot(., aes(x=ratio.x, y=ratio.y)) + geom_point() + geom_abline(intercept=0)
```

two-tailed
```{r}
wilcox.test(paired_type$ratio.x, paired_type$ratio.y, paired=TRUE)
