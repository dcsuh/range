...
```{r}
host_names <- colnames(host_para_mat[2:463])
ecoreg_names <- colnames(ecoreg_host_mat[2:717])
...
for(i in 1:length(host_para_mat)){
  parasite <- host_para_mat[i,1]
  para_pres <- host_para_mat[i,]
  para_pres %<>% pivot_longer(all_of(host_names)) %>% filter(value==1)
  host_pres <- ecoreg_host_mat %>% mutate(para=as.character(parasite), .after=sci_name) %>% filter(sci_name %in% c(para_pres$name))
  for(j in 1:length(host_pres)){
    para_ecoreg_pres <- host_pres[j,]
    para_ecoreg_pres %<>% pivot_longer(all_of(ecoreg_names)) %>% filter(value==1)
    final_para_pres <- para_ecoreg_pres %>% slice_sample(prop = null_proportion)
