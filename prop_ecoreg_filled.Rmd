...
```{r}
gmpd_type_dummy <- gmpd_type %>% 
  dplyr::select(-c(ParMacroMicro)) %>% 
  mutate(helminth = ifelse(ParType == "Helminth", 1, 0),
         arthropod = ifelse(ParType == "Arthropod", 1, 0),
         bacteria = ifelse(ParType == "Bacteria", 1, 0),
         virus = ifelse(ParType == "Virus", 1, 0),
         protozoa = ifelse(ParType == "Protozoa", 1, 0))
```
...
gmpd_traits %<>% left_join(., gmpd_type_dummy)
...
gmpd_mode %<>% dplyr::select(para, mode)
```
...
nri %<>% dplyr::select(para, mpd.obs.z)

gmpd_traits %<>% left_join(., nri)
...
gmpd_prev_cont <- gmpd %>% 
  ...
  dplyr::select(para, Prevalence)

gmpd_traits %<>% left_join(., gmpd_prev_cont)
...
gmpd_int_cont <- gmpd %>%
  ...
  rename(para = ParasiteCorrectedName) %>% 
  dplyr::select(para, Intensity)

gmpd_traits %<>% left_join(., gmpd_int_cont)
...
```{r}
gmpd_traits %<>% dplyr::select(-c(ParasiteTraitsCitation, ParType)) %>% distinct()
saveRDS(gmpd_traits, file = here("data", "processed_data", "para_traits.rds"))
```


```{r}
para_class_data <- null_para_df %>% dplyr::select(-c(sci_name, name, value)) %>% distinct()
saveRDS(para_class_data, file = here("data", "processed_data", "para_class.rds"))
```
...
```{r}
get_violin <- function(df){
  df %>% 
    filter(ratio<1) %>% 
    ggplot(., aes(x=binary, y=ratio)) + 
    geom_violin() + 
    #labs(x = "variable", y = "Actual/Theoretical PSR") + 
    theme_bw() +
    theme(axis.title = element_blank()) 
}

get_violin_alt <- function(df){
  df %>% 
...
type_plot <- get_violin(type_ratios) + geom_violin(fill = "#D81B60") + ylim(0, 0.3) + labs(title = "Type")
mode_plot <- get_violin(mode_ratios) + geom_violin(fill = "#1E88E5") + labs(title = "Mode")
spec_plot <- get_violin(spec_ratios) + geom_violin(fill = "#FFC107") + labs(title = "Specialism")
prev_plot <- get_violin(prev_ratios) + geom_violin(fill = "#004D40") + ylim(0, 0.3) + labs(title = "Prevalence")
int_plot <- get_violin(int_ratios) + geom_violin(fill = "#D01BD6") + ylim(0, 0.3) + labs(title = "Intensity")

violin_panel <- type_plot + prev_plot + int_plot + mode_plot + spec_plot 
violin_panel

# ggsave(here("figures", "type_violin.png"), type_plot)
# ggsave(here("figures", "mode_violin.png"), mode_plot)
# ggsave(here("figures", "spec_violin.png"), spec_plot)
# ggsave(here("figures", "prev_violin.png"), prev_plot)
# ggsave(here("figures", "int_violin.png"), int_plot)
# ggsave(here("figures", "violin_panel.png"), violin_panel)
```

#get means

```{r}
type_mean <- type_ratios %>% filter(ratio<1) %>% group_by(binary) %>% summarize(mean = mean(ratio))
mode_mean <- mode_ratios %>% filter(ratio<1) %>% group_by(binary) %>% summarize(mean = mean(ratio))
spec_mean <- spec_ratios %>% filter(ratio<1) %>% group_by(binary) %>% summarize(mean = mean(ratio))
prev_mean <- prev_ratios %>% filter(ratio<1) %>% group_by(binary) %>% summarize(mean = mean(ratio))
int_mean <- int_ratios %>% filter(ratio<1) %>% group_by(binary) %>% summarize(mean = mean(ratio))
...
type_plot <- get_violin_alt(type_ratios) + geom_violin(fill = "#D81B60") + ylim(0, 0.3) + labs(title = "Type")
mode_plot <- get_violin_alt(mode_ratios) + geom_violin(fill = "#1E88E5") + labs(title = "Mode")
spec_plot <- get_violin_alt(spec_ratios) + geom_violin(fill = "#FFC107") + labs(title = "Specialism")
prev_plot <- get_violin_alt(prev_ratios) + geom_violin(fill = "#004D40") + ylim(0, 0.3) + labs(title = "Prevalence")
int_plot <- get_violin_alt(int_ratios) + geom_violin(fill = "#D01BD6") + ylim(0, 0.3) + labs(title = "Intensity")
...
# ggsave(here("figures", "prev_violin.png"), prev_plot)
# ggsave(here("figures", "int_violin.png"), int_plot)
# ggsave(here("figures", "violin_panel_no_zeroes.png"), violin_panel)
```
...
```{r}
wilcox.test(ratio ~ binary, data = type_ratios %>% filter(ratio<1 & ratio>0))
wilcox.test(ratio ~ binary, data = mode_ratios %>% filter(ratio<1 & ratio>0))
wilcox.test(ratio ~ binary, data = spec_ratios %>% filter(ratio<1 & ratio>0))
wilcox.test(ratio ~ binary, data = prev_ratios %>% filter(ratio<1 & ratio>0))
wilcox.test(ratio ~ binary, data = int_ratios %>% filter(ratio<1 & ratio>0))
```
...
Same thing by parasite type

```{r}

  actual_ecoreg_arth <- ecoreg_para_mat_vars %>% 
    filter(ParType == "Arthropod") %>% 
    dplyr::select(-group_vars) %>%
    pivot_longer(all_of(ecoreg_names)) %>%
    filter(value == TRUE) %>%
    group_by(para) %>%
  ...
  actual_ecoreg_helm <- ecoreg_para_mat_vars %>% 
    filter(ParType == "Helminth") %>% 
    dplyr::select(-group_vars) %>%
    pivot_longer(all_of(ecoreg_names)) %>%
    filter(value == TRUE) %>%
    group_by(para) %>%
  ...
  actual_ecoreg_bact <- ecoreg_para_mat_vars %>% 
    filter(ParType == "Bacteria") %>% 
    dplyr::select(-group_vars) %>%
    pivot_longer(all_of(ecoreg_names)) %>%
    filter(value == TRUE) %>%
    group_by(para) %>%
  ...
  actual_ecoreg_virus <- ecoreg_para_mat_vars %>% 
  ...
  actual_ecoreg_prot <- ecoreg_para_mat_vars %>% 
  ...
  max_ecoreg_arth <- null_para_df %>% 
    filter(ParType == "Arthropod") %>% 
    dplyr::select(para, name) %>% 
    group_by(para) %>% distinct() %>% 
    summarize(n_ecoreg = n())
  
  max_ecoreg_helm <- null_para_df %>% 
    filter(ParType == "Helminth") %>% 
    dplyr::select(para, name) %>% 
    group_by(para) %>% distinct() %>% 
    summarize(n_ecoreg = n())
  
  max_ecoreg_bact <- null_para_df %>% 
    filter(ParType == "Bacteria") %>% 
    dplyr::select(para, name) %>% 
    group_by(para) %>% distinct() %>% 
    summarize(n_ecoreg = n())
  
  max_ecoreg_virus <- null_para_df %>% 
    filter(ParType == "Virus") %>% 
    dplyr::select(para, name) %>% 
    group_by(para) %>% distinct() %>% 
    summarize(n_ecoreg = n())
  
  max_ecoreg_prot <- null_para_df %>% 
    filter(ParType == "Protozoa") %>% 
    dplyr::select(para, name) %>% 
    group_by(para) %>% distinct() %>% 
    summarize(n_ecoreg = n())
  ...
  max_ecoreg_arth %<>% 
    left_join(., actual_ecoreg_arth, by=c("para")) %>% 
    mutate(
      n_ecoreg.y = replace_na(n_ecoreg.y, 0),
      class = "Arthropod")
  
  max_ecoreg_helm %<>% 
    left_join(., actual_ecoreg_helm, by=c("para")) %>% 
    mutate(
      n_ecoreg.y = replace_na(n_ecoreg.y, 0),
      class = "Helminth")
  
  max_ecoreg_bact %<>% 
    left_join(., actual_ecoreg_bact, by=c("para")) %>% 
    mutate(
      n_ecoreg.y = replace_na(n_ecoreg.y, 0),
      class = "Bacteria")
  
  max_ecoreg_virus %<>% 
    left_join(., actual_ecoreg_virus, by=c("para")) %>% 
    mutate(
      n_ecoreg.y = replace_na(n_ecoreg.y, 0),
      class = "Virus")
  
  max_ecoreg_prot %<>% 
    left_join(., actual_ecoreg_prot, by=c("para")) %>% 
    mutate(
      n_ecoreg.y = replace_na(n_ecoreg.y, 0),
      class = "Protozoa")
  
  para_type_ecoreg <- rbind(max_ecoreg_arth, 
                              max_ecoreg_helm,
                              max_ecoreg_bact,
                              max_ecoreg_virus,
                              max_ecoreg_prot)
  
  para_type_ecoreg %<>% mutate(ratio = n_ecoreg.y/n_ecoreg.x)
    
```

```{r}
para_type_ecoreg %>% ggplot(., aes(x=class, y=ratio)) + geom_boxplot()
para_type_ecoreg %>% ggplot(., aes(x=class, y=ratio)) + geom_violin()
```

```{r}
pairwise.wilcox.test(para_type_ecoreg$ratio, para_type_ecoreg$class)
```
