...
ecoreg_filled %>% filter(!is.na(ParMacroMicro)) %>%
  ggplot(., aes(x=ParMacroMicro, y=ratio)) + geom_boxplot()

ecoreg_filled %>% filter(!is.na(mode)) %>%
  ggplot(., aes(x=mode, y=ratio)) + geom_boxplot()

ecoreg_filled %>% filter(!is.na(spec_bin)) %>%
  ggplot(., aes(x=spec_bin, y=ratio)) + geom_boxplot()

ecoreg_filled %>% filter(!is.na(prev_bin)) %>%
  ggplot(., aes(x=prev_bin, y=ratio)) + geom_boxplot()

ecoreg_filled %>% filter(!is.na(int_bin)) %>%
  ggplot(., aes(x=int_bin, y=ratio)) + geom_boxplot()
```
