...
worldclim_mean %<>% mutate(ecoreg=ecoreg_pos$ECO_CODE[1], # add name of ecoregion
              na = n_missing, # number of rows with missing data
              area_km2 = total_area) # area of ecoregion (should check units)

for (i in 2:dim(ecoreg_pos)[1]){ # same as step above but looped over the remaining 2:n ecoregions
  ...
  worldclim_mean_tmp %<>% mutate(ecoreg=ecoreg_pos$ECO_CODE[i],
                na = n_missing,
                area_km2 = total_area)
