...
```{r}
gmpd %<>% drop_na(., Longitude, Latitude) %>% st_as_sf(., coords=c("Longitude", "Latitude"), crs = st_crs(ecoregions))
```

```{r}
ecoregions %>% st_simplify() %>% tm_shape(.) + tm_borders()
```


```{r}
tm_shape(gmpd) + tm_dots()
```
