...
para_traits <- readRDS(here("data", "processed_data", "para_traits.rds"))
pres_abs_traits_df <- pres_abs_df %>% left_join(., para_traits, by="para")
...
train <- sample(1:nrow(pres_abs_traits_df), nrow(pres_abs_traits_df) * 0.8)
test <- pres_abs_traits_df %>% mutate(row_num = rownames(.)) %>% filter(!row_num %in% train) %>% dplyr::select(row_num)
test <- as.integer(test$row_num)

pres_abs_traits_df_train <- pres_abs_traits_df[train, ]
pres_abs_traits_df_test <- pres_abs_traits_df[test, ]
```



```{r, eval=F}
brt5 <- gbm.step(as.data.frame(pres_abs_traits_df_train), 
              gbm.x = c(4:ncol(pres_abs_traits_df_train)), 
              gbm.y = 3, 
              family = "bernoulli", 
              tree.complexity = 4, 
              learning.rate = 0.05, 
...
saveRDS(brt5, file = here("data", "processed_data", "brt", "brt5_nonbinary_lr0.05.rds"))
```

```{r}
summary(brt5)
gbm.plot(brt5, n.plots=9, plot.layout=c(3, 3), write.title = FALSE, common.scale = T)
```


```{r}
pres_abs_traits_df %<>% dplyr::select(-c(effort, avail_hosts))
# %<>%
#   filter(ParType %in% c("Arthropod", "Helminth", "Virus", "Bacteria", "Protozoa")) %>%
#   mutate(ParType = as.factor(ParType),
#          ParMacroMicro = as.factor(ParMacroMicro),
...
```{r, eval=F}
brt6 <- gbm.step(as.data.frame(pres_abs_traits_df_train), 
              gbm.x = c(4:ncol(pres_abs_traits_df_train)), 
              gbm.y = 3, 
              family = "bernoulli", 
              tree.complexity = 4, 
...
saveRDS(brt6, file = here("data", "processed_data", "brt", "brt5_nonbinary_noeffort_lr0.05.rds"))
```

```{r}
summary(brt6)
gbm.plot(brt6, n.plots=6, plot.layout=c(2, 3), write.title = FALSE, common.scale = T)
gbm.plot(brt6, n.plots=9, plot.layout=c(3, 3), write.title = FALSE, common.scale = T)
```
